...
gpModel <- nhlModel(2012, 2013, 3, seed = 798619)
...
dev.off()
gpTest <- nhlModel(2009, 2010, 3, cols = cols, seed = 794825)
corData <- nhlShape(2009, 2010, cols = cols, outcome = 3, rm.nhlnum = FALSE)
...
qplot(outcome, games_played, data = corData, geom = c("smooth", "point"),
      ...
      xlab = "Predicted Games Played", ylab = "Actual Games Played")
dev.off()
testData <- nhlShape(2010, 2011, cols = cols, outcome = 3, rm.nhlnum = FALSE)
testData <- merge(testData, nhlPredict(2010, 2011, 3, gpTest))
...
test2Data <- nhlShape(2011, 2012, cols = cols, outcome = 3, rm.nhlnum = FALSE)
test2Data <- merge(test2Data, nhlPredict(2011, 2012, 3, gpTest))
